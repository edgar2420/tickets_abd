  ...
  if (permisos.length > 0 && !puede(...permisos)) {
    return (
      <div className="panel mx-auto max-w-lg p-8 text-center">
        ...
        <h2 className="mt-3 text-lg font-bold text-institucional-900">Acceso restringido</h2>
        <p className="mt-1 text-sm text-slate-600">
          Su rol no cuenta con el permiso requerido para consultar esta seccion.
        </p>
        <p className="mt-3 text-xs text-slate-400">Permiso requerido: {permisos.join(' o ')}</p>
      </div>
    );
  }
